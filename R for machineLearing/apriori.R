setwd("C:\\Users\\user\\Desktop\\빅데이터분석기사\\apriori")
...
#head(inspect(grocerie)) #하면 전체 트랜잭션 나옴
...
fruitrules=subset(grocerirules,items %in% "fruit")#에러 tropical fruit 이런 식이라
fruitrules=subset(grocerirules,items %pin% "fruit") #partial in으로 하면 나옴
inspect(fruitrules)

#베리, 요거트 동시 포함
#subset은 설정하는 조건에 맞는 벡터, 매트릭스 혹은 데이터 프레임 반환
